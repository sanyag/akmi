 ...
 * for you.
 */
//$base_url = 'http://akdndev.prod.acquia-sites.com/museum';  // NO trailing slash!
...
if (file_exists('/var/www/site-php')) {
  require('/var/www/site-php/akdn/akdnmuseum-settings.inc');

  $shared = $databases['akdnusers']['default']['database'] . '.';
  ...
  acquia_hosting_db_choose_active();
   ...
   // Create a per-environment hash for Apache Solr.
   ...
   // The hash denotes the site, or environment, so we have to ensure it is different for each stage.
   $hash = sha1(serialize($databases) . $env . $base_url); # e.g. xyz12dev
   $hash = base_convert($hash,16,36);
   $conf['apachesolr_site_hash'] = substr($hash, 0, 6);
   
	// Enable memcache.
	$conf['cache_backends'][] = './sites/museum/modules/contrib/memcache/memcache.inc';
 ...
 * Include Variable overrides for a local envrionment
 **/

$lp = dirname(__FILE__) . "/settings.local.php";
